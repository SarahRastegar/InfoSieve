...
if __name__ == "__main__":
    ...
    if args.mytraining:
        ...
        args.save_dir+='/Comb'
        print('Performing SS-K-Means on all in the training data using Combinations...')
        all_acc, old_acc, new_acc, kmeans_comb = test_kmeans_semi_sup(train_loader, args, K=args.K, silence=args.silent, unbalanced =args.unbalanced)
        cluster_save_path = os.path.join(args.save_dir, 'ss_kmeans_cluster_centres.pt')
        torch.save(kmeans_comb .cluster_centers_, cluster_save_path)
        ...
        old_save_dir =args.save_dir
        args.save_dir+='/Labels'
        print('Performing SS-K-Means on all in the training data using Labels...')
        all_acc, old_acc, new_acc, kmeans_lab = test_kmeans_semi_sup(train_loader, args, K=args.K, silence=args.silent, is_code=True, unbalanced =args.unbalanced)
        cluster_save_path = os.path.join(args.save_dir, 'ss_kmeans_cluster_centres.pt')
        torch.save(kmeans_lab.cluster_centers_, cluster_save_path)
        args.save_dir=old_save_dir
        f.write(str(all_acc) + '\n')
        f.write(str(old_acc) + '\n')
        f.write(str(new_acc) + '\n')
